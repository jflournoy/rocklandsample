...
knitr::kable(arrange(param_sig_freq_df, Var1), col.names = c('Sigificant in ...', 'Linear Age', 'Quadratic Age'))
knitr::kable(agreement, col.names = c('Structure and Hemisphere', 'Linear Age', 'Quadratic Age'))
...
nada <- model_estimat_stats %>%
  ...
  do({
    ...
    xlims <- quantile(.$perc_diff, probs = c(.025, .975))
    xlims[1] <- ifelse(xlims[1] < 0, xlims[1], 0)
    aplot <- ggplot(., aes(x=perc_diff))+
      ...
      geom_vline(xintercept = xmedian, color = 'red')+
