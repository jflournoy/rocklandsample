...
power_to_detect <- model_fe %>% 
  ...
  mutate(p_lib = quick_t_p(t = `t value`, df = nobs - 3),
         p_cons = quick_t_p(t = `t value`, df = ngrps - 3),
         sig_lib = p_lib < .05,
         sig_cons = p_cons < .05) %>%
  select(-`t value`, -nobs, -ngrps, -p_lib, -p_cons) %>%
  gather(stat, value,  sig_lib, sig_cons) %>%
  spread(data_set, value) %>%
  mutate(disagreement = avail != cc)
...
model_fe %>%
  ...
  geom_density(aes(group = data_set, fill = data_set),
                 alpha = .5)+
